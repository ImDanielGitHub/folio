...
- The concurrent target bootstrap records Apache-2.0 as the intended licence for original code, but its current `LICENSE` is explicitly provisional and `package.json` remains `UNLICENSED`. Until Daniel confirms ownership and replaces that notice with an effective licence, the repository is **not yet open source** and must not be described or published as such. Apache-2.0 original code can coexist with properly noticed MIT-derived portions, subject to a final licence review.
