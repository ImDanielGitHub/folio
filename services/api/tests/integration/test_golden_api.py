...
async def test_real_golden_api_flow_is_local_exact_and_reversible(tmp_path: Path) -> None:
    ...
    async with httpx.AsyncClient(
        ...
        akahu_body = akahu.json()
        assert akahu_body["liveSyncAttempted"] is False
        assert akahu_body["rowCount"] == 6
        assert akahu_body["status"] in {"ingested", "deduplicated"}
        sources = (
            await client.get("/v1/workspaces/ws_koru_studio/snapshot")
        ).json()["sources"]
        assert any(item["sourceType"] == "akahu_fixture" for item in sources)
