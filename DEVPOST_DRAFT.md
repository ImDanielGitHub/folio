...
Folio is not Hermes, and it is not a chat wrapper over opaque cloud finance. Hermes donated connector ideas; Folio is a separate local-first operator with conversation as home and a dynamic canvas only when needed. A local model is optional—money totals, classifications, forecasts and documents stay deterministic and never come from model prose. Every material change ships with linked evidence and a visible Undo. Against Bionic (harness-only) and ChatGPT Finance (cloud/opaque), Folio’s differentiator is sealed local finance state you can audit and reverse.

The Build Week prototype runs a complete fictional small-business workflow. It ingests a bank CSV, performs a Daily Close, detects a likely duplicate and an expense that needs context, calculates a 30-day cash scenario, and prepares an evidence-linked owner pack. The owner can explain a transaction naturally, see Folio update its understanding, and undo the resulting classification change without losing the history.
