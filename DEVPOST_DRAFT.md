...
The interface hides that complexity. The dark desktop shell is a continuing conversation beside a stateful canvas. Charts, records, sources and the owner pack open only when the current question needs them. Evidence is available from compact source links; raw tool and model diagnostics stay in a calm tool-trace surface. When LM Studio is unavailable, Folio stays on deterministic fallback and never silently calls the cloud.
