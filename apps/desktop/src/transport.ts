...
export type RuntimeMode = "checking" | "live" | "fixture" | "offline";
...
export async function ingestAkahuFixture(payload: AkahuFixturePayload = {}): Promise<Record<string, unknown>> {
  // Empty body loads the sealed Koru Akahu fixture on the API.
  try {
    const body: Record<string, unknown> = {};
    if (payload.syncedAt) body.syncedAt = payload.syncedAt;
    if (payload.transactions && payload.transactions.length > 0) {
      body.transactions = payload.transactions;
      body.account = { name: payload.account ?? "ANZ Everyday", currency: "NZD" };
    }
    return await requestJson("/v1/ingest/akahu-fixture", {
      method: "POST",
    ...
    return {
      mode: "fixture",
      status: "ingested",
      account: payload.account ?? "anz_everyday",
      syncedAt: payload.syncedAt ?? new Date().toISOString(),
      sourceItemId: "src_koru_akahu_anz_everyday",
      rowCount: payload.transactions?.length || 6,
    };
  }
}
